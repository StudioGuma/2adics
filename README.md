...
Building requires installing a compiler, preferably GCC, and Make on Windows. To build, open a command line and type `make`. You can then execute by typing `2adics <a> <b>` (on Windows) or `./2adics <a> <b>` (on Unix-like systems), where `<a>` and `<b>` are the numerator and denominator inputs, and clean by typing `make clean` after editing the makefile based on your OS.
