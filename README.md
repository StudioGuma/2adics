...
Building requires a compiler, preferably GCC, as well as Git or GNU Make on Windows. To build, open a command line and type `make`. You can then execute by typing `./2adics <a> <b>` (on Unix-like systems) or `2adics <a> <b>` (on Windows), where `<a>` and `<b>` are the numerator and denominator inputs, and clean by typing `make clean`. Make sure to edit the makefile according to your compiler and OS.
